  ...
  switch (action.type) {
    ...
    case actionTypes.GET_FILTERS_CATEGORY:
      const filterCategories = state.Shoes.filter((product) => {
        ...
        if (action.payload.toLowerCase() === product.category.toLowerCase())
          return product;
        return false;
      });
    ...
    case actionTypes.GET_FILTERS_BRANDS:
      const filterBrands = state.Shoes.filter((product) => {
        if (action.payload === "All") return product;
        if (action.payload.toLowerCase() === product.brand_name.toLowerCase())
          return product;
        return false;
      });
    ...
    case actionTypes.UPDATE_FILTERS:
      ...
      const productsBrands = productSelects.filter((product) => {
        ...
        if (typeof brands === "string")
          return brands.toLowerCase() === product.brand_name.toLowerCase();
      ...
      const productsGenders = productsBrands.filter((product) => {
        ...
        if (typeof genders === "string")
          return product.genders.includes(genders);
